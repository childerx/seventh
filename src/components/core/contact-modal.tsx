...
const ContactModal: React.FC = () => {
  ...
  return (
    <Modal isOpen={isContactModalOpen} onClose={closeContactModal} title="Get In Touch">
      <p className="mb-6" style={{ color: isDark ? "#94a3b8" : "#64748b" }}>
        Ready to ship? Fill in your details and we'll get back to you with a tailored quote.
      </p>
      ...
      <div className="space-y-5 mb-8">
        {contactInfo.map((info, idx) => (
          ...
          >
            ...
            <div>
              <h3 className="font-semibold text-sm mb-1" style={{ color: isDark ? "#e2e8f0" : "#1e293b" }}>
                {info.title}
              </h3>
              {info.items.map((item, i) => (
                info.href ? (
                  <a key={i} href={info.href} className="text-sm hover:text-blue-500 transition-colors" style={{ color: isDark ? "#94a3b8" : "#64748b" }}>
                    {item}
                  </a>
                ) : (
                  <p key={i} className="text-sm" style={{ color: isDark ? "#94a3b8" : "#64748b" }}>
                    {item}
                  </p>
                )
              ))}
