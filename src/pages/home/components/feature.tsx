...
const features = [
  ...
  },
  {
    ...
    description:
      "Full insurance coverage and maximum security handling throughout the entire journey.",
    image: planeCargo1,
    detail:
      "Security is the cornerstone of Seventh Air's shipping philosophy. Every consignment is covered by our comprehensive insurance policy, underwritten by Lloyd's of London, providing full indemnity against loss, damage, or theft from collection to delivery. Our warehousing facilities employ 24/7 CCTV surveillance, biometric access control, and tamper-evident sealing protocols. During transit, all cargo is secured within ULD (Unit Load Device) containers that are tracked at every waypoint. Our security-cleared personnel handle each package with rigorous chain-of-custody procedures, ensuring an unbroken audit trail from sender to recipient.",
  },
  {
    title: "Real-time Tracking",
    icon: <Clock className="w-7 h-7" />,
    description:
      "Advanced tracking system with instant updates on your shipment's location and status.",
    image: planeCargo2,
    detail:
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="relative">
          <button
            onClick={() => setActiveCard((p) => (p === 0 ? 3 : p - 1))}
            className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-3 lg:-translate-x-12 z-20 p-2 lg:p-3 rounded-full shadow-lg transition-all hover:scale-110"
            style={{
              background: isDark
                ? "rgba(30, 41, 59, 0.9)"
          ...
          <button
            onClick={() => setActiveCard((p) => (p === 3 ? 0 : p + 1))}
            className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-3 lg:translate-x-12 z-20 p-2 lg:p-3 rounded-full shadow-lg transition-all hover:scale-110"
            style={{
              background: isDark
                ? "rgba(30, 41, 59, 0.9)"
                : "rgba(255, 255, 255, 0.9)",
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <motion.button
                      className="inline-flex items-center space-x-2 px-4 py-2 rounded-full font-semibold text-xs"
                      style={{
                        background:
                  ...
                  <div
                    className="absolute -top-2 -right-2 w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-md z-10"
                    style={{
                      background:
                        "linear-gradient(135deg, #1e40af 0%, #dc2626 100%)",
                    }}
          ...
          <div className="flex justify-center space-x-2 mt-8 sm:mt-12">
            {features.map((_, index) => (
              <button
                ...
                style={{
                  width: activeCard === index ? "2rem" : "0.5rem",
                  height: "0.5rem",
                  background:
                    activeCard === index
                      ? "linear-gradient(135deg, #1e40af 0%, #dc2626 100%)"
